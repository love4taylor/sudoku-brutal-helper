        ...
        match write_rule(&format!("del {prefix}\n")) {
            Ok(()) => {
                if let Err(error) = route_delete(ip) {
                    warn!("failed to delete the Brutal route for {}; continuing: {error:#}", prefix);
                }
                Ok(true)
            }
            Err(error) => {
...
fn diag_dump(
    ...
    put_nl_header(&mut request, SOCK_DIAG_BY_FAMILY, NLM_F_REQUEST | NLM_F_DUMP, 1);
    request[16] = family;
...
fn decode_socket_identity(payload: &[u8], family: u8) -> Result<SocketIdentity> {
    ...
    Ok(SocketIdentity { local, peer, local_port, peer_port, cookie })
}

fn decode_ip(family: u8, bytes: &[u8]) -> Result<IpAddr> {
    match family as i32 {
        AF_INET => Ok(IpAddr::V4(Ipv4Addr::new(bytes[0], bytes[1], bytes[2], bytes[3]))),
        AF_INET6 => Ok(IpAddr::V6(Ipv6Addr::from(<[u8; 16]>::try_from(&bytes[..16]).unwrap()))),
...
struct NetlinkMessage { kind: u16, payload: Vec<u8> }

struct NetlinkSocket { fd: RawFd }
...
impl NetlinkSocket {
    fn open(protocol: i32, timeout: Duration) -> Result<Self> {
        ...
        let result = unsafe {
            libc::bind(fd, (&address as *const libc::sockaddr_nl).cast(), size_of::<libc::sockaddr_nl>() as u32)
        };
        if result < 0 {
    ...
    fn send(&self, message: &[u8]) -> Result<()> {
        ...
        address.nl_family = AF_NETLINK as u16;
        let sent = unsafe {
            libc::sendto(self.fd, message.as_ptr().cast(), message.len(), 0, (&address as *const libc::sockaddr_nl).cast(), size_of::<libc::sockaddr_nl>() as u32)
        };
        if sent < 0 {
    ...
    fn receive_messages(&self) -> Result<Vec<NetlinkMessage>> {
        ...
        loop {
            ...
            if received < 0 {
                return Err(std::io::Error::last_os_error()).context("failed to read netlink response");
            }
            if received == 0 {
                break;
            }
            ...
            while offset + 16 <= received as usize {
                let length = u32::from_ne_bytes(buffer[offset..offset + 4].try_into().unwrap()) as usize;
                ...
                messages.push(NetlinkMessage { kind, payload: buffer[offset + 16..offset + length].to_vec() });
                offset += (length + 3) & !3;
                if kind == NLMSG_DONE || kind == NLMSG_ERROR { return Ok(messages); }
            }
        }
        Ok(messages)
    }
...
fn route_replace(ip: IpAddr) -> Result<()> {
    ...
    if route.local { bail!("destination {ip} is a local address"); }
    let mut payload = vec![0u8; 12];
    payload[0] = ip_family(ip); payload[1] = prefix_len(ip); payload[4] = RT_TABLE_MAIN;
    payload[5] = BRUTAL_ROUTE_PROTOCOL; payload[6] = RT_SCOPE_UNIVERSE; payload[7] = RTN_UNICAST;
    append_attr(&mut payload, RTA_DST, &ip_bytes(ip));
    if let Some(gateway) = route.gateway { append_attr(&mut payload, RTA_GATEWAY, &ip_bytes(gateway)); }
    append_attr(&mut payload, RTA_OIF, &route.interface.to_ne_bytes());
    let mut metrics = Vec::new();
    append_attr(&mut metrics, RTAX_LOCK, &(1u32 << RTAX_CC_ALGO).to_ne_bytes());
    append_attr(&mut metrics, RTAX_CC_ALGO, b"brutal\0");
    append_attr(&mut payload, RTA_METRICS, &metrics);
    route_request(RTM_NEWROUTE, NLM_F_REQUEST | NLM_F_ACK | NLM_F_CREATE | 0x100, payload)
}

fn route_delete(ip: IpAddr) -> Result<()> {
    let mut payload = vec![0u8; 12];
    payload[0] = ip_family(ip); payload[1] = prefix_len(ip); payload[4] = RT_TABLE_MAIN;
    payload[5] = BRUTAL_ROUTE_PROTOCOL; payload[7] = RTN_UNICAST;
    append_attr(&mut payload, RTA_DST, &ip_bytes(ip));
    route_request(RTM_DELROUTE, NLM_F_REQUEST | NLM_F_ACK, payload)
}

struct RouteLookup { gateway: Option<IpAddr>, interface: u32, local: bool }
...
fn route_lookup_ipv4(ip: Ipv4Addr) -> Result<RouteLookup> {
    ...
    for line in contents.lines().skip(1) {
        ...
        let gateway_raw = u32::from_str_radix(fields[2], 16).context("invalid IPv4 route gateway")?;
        ...
        if target & mask != destination { continue; }
        let prefix_len = mask.count_ones();
        let metric = fields.get(6).and_then(|value| value.parse::<u32>().ok()).unwrap_or(u32::MAX);
        ...
        if replace {
            ...
            let interface = unsafe { libc::if_nametoindex(std::ffi::CString::new(fields[0]).unwrap().as_ptr()) };
    ...
    Ok(RouteLookup { gateway: gateway.map(IpAddr::V4), interface, local: is_local_address(IpAddr::V4(ip)) })
}
...
fn route_lookup_ipv6(ip: Ipv6Addr) -> Result<RouteLookup> {
    ...
    let contents = read_to_string("/proc/net/ipv6_route").context("failed to read /proc/net/ipv6_route")?;
    for line in contents.lines() {
        ...
        if prefix_len > 128 || !prefix_matches(target, destination, prefix_len) { continue; }
        ...
        if replace {
            let interface = unsafe { libc::if_nametoindex(std::ffi::CString::new(fields[9]).unwrap().as_ptr()) };
            if interface == 0 { continue; }
    ...
    Ok(RouteLookup { gateway: gateway.map(IpAddr::V6), interface, local: is_local_address(IpAddr::V6(ip)) })
}

fn parse_hex_ip6(value: &str) -> Result<Ipv6Addr> {
    if value.len() != 32 { bail!("invalid IPv6 route address length"); }
    let mut bytes = [0u8; 16];
    for (index, chunk) in value.as_bytes().chunks_exact(2).enumerate() {
        bytes[index] = u8::from_str_radix(std::str::from_utf8(chunk).unwrap(), 16).context("invalid IPv6 route address")?;
    }
...
fn is_local_address(ip: IpAddr) -> bool {
    ...
    if unsafe { libc::getifaddrs(&mut addresses) } != 0 { return false; }
    ...
    while !current.is_null() {
        ...
        if !entry.ifa_addr.is_null() {
            ...
            if (family == AF_INET || family == AF_INET6) && sockaddr_ip(entry.ifa_addr, family).is_some_and(|address| address == ip) {
                found = true;
                break;
            }
        }
...
fn sockaddr_ip(address: *const libc::sockaddr, family: i32) -> Option<IpAddr> {
    if family == AF_INET {
        ...
        Some(IpAddr::V4(Ipv4Addr::from(u32::from_be(address.sin_addr.s_addr).to_be_bytes())))
...
fn route_request_response(kind: u16, flags: u16, payload: Vec<u8>) -> Result<Vec<u8>> {
    ...
    put_nl_header(&mut request, kind, flags, 1); request[16..].copy_from_slice(&payload);
    let socket = NetlinkSocket::open(NETLINK_ROUTE, Duration::from_secs(15))?;
    socket.send(&request)?;
    for message in socket.receive_messages()? {
        if message.kind == NLMSG_ERROR {
            if message.payload.len() < 4 { bail!("route netlink returned an invalid error"); }
            let error = i32::from_ne_bytes(message.payload[0..4].try_into().unwrap());
            if error == 0 { return Ok(Vec::new()); }
            return Err(std::io::Error::from_raw_os_error(-error).into());
        }
        if message.kind != NLMSG_NOOP { return Ok(message.payload); }
    }
...
fn append_attr(buffer: &mut Vec<u8>, kind: u16, value: &[u8]) {
    ...
    buffer.extend_from_slice(&kind.to_ne_bytes()); buffer.extend_from_slice(value);
...
fn parse_attrs(mut bytes: &[u8]) -> Result<Vec<(u16, Vec<u8>)>> {
    ...
    while bytes.len() >= 4 {
        ...
        if length < 4 || length > bytes.len() { bail!("route netlink returned an invalid attribute"); }
        let kind = u16::from_ne_bytes(bytes[2..4].try_into().unwrap());
        attrs.push((kind, bytes[4..length].to_vec()));
        let aligned = (length + 3) & !3; if aligned > bytes.len() { break; } bytes = &bytes[aligned..];
    }
    Ok(attrs)
}

fn ip_family(ip: IpAddr) -> u8 { if ip.is_ipv4() { AF_INET as u8 } else { AF_INET6 as u8 } }
fn prefix_len(ip: IpAddr) -> u8 { if ip.is_ipv4() { 32 } else { 128 } }
fn ip_bytes(ip: IpAddr) -> Vec<u8> { match ip { IpAddr::V4(ip) => ip.octets().to_vec(), IpAddr::V6(ip) => ip.octets().to_vec() } }
...
mod tests {
    ...
    fn formats_host_prefixes() {
        ...
        assert_eq!(client_prefix("2001:db8::1".parse().unwrap()), "2001:db8::1/128");
    }

    #[test]
    fn appends_aligned_route_attributes() {
        let mut bytes = Vec::new(); append_attr(&mut bytes, RTA_OIF, &7u32.to_ne_bytes()); append_attr(&mut bytes, RTA_GATEWAY, &[192, 0, 2, 1]);
        let attrs = parse_attrs(&bytes).unwrap(); assert_eq!(attrs[0].1, 7u32.to_ne_bytes()); assert_eq!(attrs[1].1, vec![192, 0, 2, 1]);
    }

    #[test]
    fn decodes_proc_ipv4_values() {
        assert_eq!(proc_ipv4_value("FAFA29C1").unwrap(), u32::from_be_bytes([193, 41, 250, 250]));
        assert_eq!(Ipv4Addr::from(u32::from_str_radix("FAFA29C1", 16).unwrap().to_le_bytes()), Ipv4Addr::new(193, 41, 250, 250));
    }
}
